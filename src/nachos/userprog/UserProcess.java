...
public class UserProcess {
	...
	public int readVirtualMemory(int vaddr, byte[] data, int offset, int length) {
		Lib.my_assert(offset >= 0 && length >= 0
	...
	private boolean load(String name, String[] args) {
		...
		for (int i = 0; i < argv.length; i++) {
			...
			Lib
					.my_assert(writeVirtualMemory(entryOffset,
							stringOffsetBytes) == 4);
			entryOffset += 4;
			Lib
					.my_assert(writeVirtualMemory(stringOffset, argv[i]) == argv[i].length);
			stringOffset += argv[i].length;
			Lib
					.my_assert(writeVirtualMemory(stringOffset,
							new byte[] { 0 }) == 1);
			stringOffset += 1;
		}
