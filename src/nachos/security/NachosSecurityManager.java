...
public class NachosSecurityManager extends SecurityManager {
	...
	public Privilege getPrivilege() {
		Lib.my_assert(this != System.getSecurityManager());
	...
	private void enablePrivilege() {
		if (privilegeCount == 0) {
			Lib.my_assert(privileged == null);
			privileged = Thread.currentThread();
			privilegeCount++;
		} else {
			Lib.my_assert(privileged == Thread.currentThread());
			privilegeCount++;
		}
	}
	...
	private void disablePrivilege() {
		Lib.my_assert(privileged != null && privilegeCount > 0);
		privilegeCount--;
		if (privilegeCount == 0)
			privileged = null;
	}
