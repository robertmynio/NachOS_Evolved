...
public final class Processor {
	...
	public int readRegister(int number) {
		Lib.my_assert(number >= 0 && number < numUserRegisters);
	...
	public TranslationEntry readTLBEntry(int number) {
		Lib.my_assert(usingTLB);
		Lib.my_assert(number >= 0 && number < tlbSize);
	...
	public static int makeAddress(int page, int offset) {
		Lib.my_assert(page >= 0 && page < maxPages);
		Lib.my_assert(offset >= 0 && offset < pageSize);
	...
	private int readMem(int vaddr, int size) throws MipsException {
		...
		Lib.my_assert(size == 1 || size == 2 || size == 4);
	...
	private class MipsException extends Exception {
		public MipsException(int cause) {
			Lib.my_assert(cause >= 0 && cause < exceptionNames.length);
		...
		public void handle() {
			...
			Lib.my_assert(exceptionHandler != null);
		...
		private void print() {
			...
			for (int i = 0; i < args.length; i++) {
				...
				if (i + 1 < args.length) {
					...
					Lib.my_assert(maxCharsPrinted - minCharsPrinted < 8);
