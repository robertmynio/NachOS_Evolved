...
public final class Lib {
	 ...
	 */
	public static void my_assert(boolean expression) {
		if (!expression)
	 ...
	 *            the error message.
	 */
	public static void my_assert(boolean expression, String message) {
		if (!expression)
			throw new AssertionFailureError(message);
	}

	/**
	 * Asserts that this call is never made. Same as <tt>my_assert(false)</tt>.
	 */
	 ...
	 * Same as <tt>my_assert(false, message)</tt>.
	 * 
	 * @param message
	 *            the error message.
	 */
	...
	public static void strictReadFile(OpenFile file, int position, byte[] buf,
		...
		my_assert(file.read(position, buf, offset, length) == length);
		long finishTime = Machine.timer().getTime();
		my_assert(finishTime > startTime);
	}
	...
	public static void bytesFromInt(byte[] array, int offset, int length,
			int value) {
		my_assert(length == 1 || length == 2 || length == 4);
	...
	public static void checkDerivation(Class cls, Class superCls) {
		Lib.my_assert(superCls.isAssignableFrom(cls));
	}
	...
	public static void checkConstructor(Class cls, Class[] parameterTypes) {
		try {
			Lib.my_assert(Modifier.isPublic(cls.getModifiers())
					&& !Modifier.isAbstract(cls.getModifiers()));
			Constructor constructor = cls.getConstructor(parameterTypes);
			Lib.my_assert(Modifier.isPublic(constructor.getModifiers()));
		} catch (Exception e) {
	...
	public static void checkMethod(Class cls, String methodName,
		...
		try {
			...
			Lib.my_assert(Modifier.isPublic(method.getModifiers())
					&& !Modifier.isStatic(method.getModifiers()));
			Lib.my_assert(method.getReturnType() == returnType);
		} catch (Exception e) {
			Lib.assertNotReached();
		}
	}
	...
	public static void checkField(Class cls, String fieldName, Class fieldType) {
		try {
			...
			Lib.my_assert(field.getType() == fieldType);
			Lib.my_assert(Modifier.isPublic(field.getModifiers())
