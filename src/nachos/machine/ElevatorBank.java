...
public final class ElevatorBank implements Runnable {
	...
	public void init(int numElevators, int numFloors,
		...
		Lib.my_assert(!simulationStarted);
	...
	public void enableGui() {
		Lib.my_assert(!simulationStarted);
		Lib.my_assert(Config.getBoolean("ElevatorBank.allowElevatorGUI"));
	...
	void postRiderEvent(int event, int floor, int elevator) {
		...
		if (elevator != -1) {
			Lib.my_assert(elevator >= 0 && elevator < numElevators);
	...
	private class ElevatorManager implements ElevatorControls {
		...
		public void openDoors(int elevator) {
			Lib.my_assert(elevator >= 0 && elevator < numElevators);
			postRiderEvent(RiderEvent.eventDoorsOpened, elevators[elevator]
					.openDoors(), elevator);
		...
		public void closeDoors(int elevator) {
			Lib.my_assert(elevator >= 0 && elevator < numElevators);
			postRiderEvent(RiderEvent.eventDoorsClosed, elevators[elevator]
					.closeDoors(), elevator);
		...
		public boolean moveTo(int floor, int elevator) {
			Lib.my_assert(floor >= 0 && floor < numFloors);
			Lib.my_assert(elevator >= 0 && elevator < numElevators);
		...
		public void finish() {
			...
			Lib.my_assert(KThread.currentThread() == thread);
	...
	private class ElevatorState {
		...
		int openDoors() {
			Lib.my_assert(!doorsOpen && !moving);
			doorsOpen = true;
			return floor;
		}

		int closeDoors() {
			Lib.my_assert(doorsOpen);
			doorsOpen = false;
			return floor;
		}
		...
		boolean moveTo(int newDestination) {
			...
			if (!moving) {
				...
				Lib.my_assert(floor != destination);
		...
		boolean enter(RiderState rider, int onFloor) {
			Lib.my_assert(!riders.contains(rider));
		...
		boolean atNextFloor() {
			...
			Lib.my_assert(destination != floor);
	...
	private class RiderState implements RiderControls {
		...
		public boolean pressUpButton() {
			Lib.my_assert(!inElevator && floor < numFloors - 1);
		...
		public boolean pressDownButton() {
			Lib.my_assert(!inElevator && floor > 0);
		...
		public boolean enterElevator(int elevator) {
			Lib.my_assert(!inElevator && elevator >= 0
		...
		public boolean pressFloorButton(int floor) {
			Lib.my_assert(inElevator && floor >= 0 && floor < numFloors);
		...
		public void finish() {
			...
			Lib.my_assert(floors.length == stops.length);
			for (int i = 0; i < floors.length; i++)
				Lib.my_assert(floors[i] == stops[i]);
