...
public final class TCB {
	...
	public void start(Runnable target) {
		 ...
		 */
		if (!isFirstTCB)
			Lib.my_assert(currentTCB.javaThread == Thread.currentThread());
	...
	public void contextSwitch() {
		...
		Lib.my_assert(currentTCB.associated);
	...
	private void threadroot() {
		...
		Lib.my_assert(javaThread == Thread.currentThread());
	...
	private void associateThread(KThread thread) {
		...
		// context switch
		Lib.my_assert(!associated);
		associated = true;

		Lib.my_assert(thread != null);

		if (nachosThread != null)
			Lib.my_assert(thread == nachosThread);
		else
			nachosThread = thread;
	}
	...
	private static void authorizeDestroy(KThread thread) {
		...
		// destroy
		Lib.my_assert(toBeDestroyed == null);
