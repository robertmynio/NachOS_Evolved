...
public final class ElevatorTest {
	...
	private class ElevatorController implements ElevatorControllerInterface {
		...
		public void run() {
			...
			Lib.my_assert(controls.getFloor(0) == 0);

			e = getNextEvent();
			Lib.my_assert(e.event == ElevatorEvent.eventUpButtonPressed
					&& e.floor == 0);
			...
			e = getNextEvent();
			Lib.my_assert(e.event == ElevatorEvent.eventFloorButtonPressed
					&& e.floor == 1);
			...
			e = getNextEvent();
			Lib.my_assert(e.event == ElevatorEvent.eventElevatorArrived
					&& e.floor == 1 && e.elevator == 0);

			controls.openDoors(0);

			e = getNextEvent();
			Lib.my_assert(e.event == ElevatorEvent.eventRidersDone);
	...
	private class Rider implements RiderInterface {
		public void initialize(RiderControls controls, int[] stops) {
			this.controls = controls;
			Lib.my_assert(stops.length == 1 && stops[0] == 1);
		...
		public void run() {
			...
			Lib.my_assert(controls.getFloor() == 0);

			controls.pressUpButton();

			e = getNextEvent();
			Lib.my_assert(e.event == RiderEvent.eventDoorsOpened
					&& e.floor == 0 && e.elevator == 0);
			Lib.my_assert(controls.getDirectionDisplay(0) == dirUp);

			Lib.my_assert(controls.enterElevator(0));
			controls.pressFloorButton(1);

			e = getNextEvent();
			Lib.my_assert(e.event == RiderEvent.eventDoorsClosed
					&& e.floor == 0 && e.elevator == 0);
			...
			Lib.my_assert(controls.exitElevator(1));
