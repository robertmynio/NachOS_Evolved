...
public class AutoGrader {
	...
	public void start(Privilege privilege) {
		Lib.my_assert(this.privilege == null, "start() called multiple times");
	...
	private void extractArguments(String[] args) {
		...
		while (st.hasMoreTokens()) {
			...
			Lib.my_assert(pair.hasMoreTokens(), "test argument missing key");
			String key = pair.nextToken();

			Lib.my_assert(pair.hasMoreTokens(), "test argument missing value");
	...
	String getStringArgument(String key) {
		...
		Lib.my_assert(value != null, "getStringArgument(" + key
				+ ") failed to find key");
		return value;
	}
	...
	void level(int level) {
		this.level++;
		Lib.my_assert(level == this.level,
	...
	public void timerInterrupt(Privilege privilege, long time) {
		Lib.my_assert(privilege == this.privilege, "security violation");
	}
